...
module.exports = class FpCommand extends (
	BaseCommand
) {
	constructor() {
		super(
			...
			"Applies given picture and with a facepalm",
			"",
			["SEND_MESSAGES", "EMBED_LINKS"],
			["fp help` - Sends the help command"],
			["fp me", "fp help", "fp @Tech!", "fp (attachment)"],
			true,
			5000,
			false,
			false,
		...
		if (args[0]) {
			if (args[0].toLowerCase() == "help") {
				...
				try {
					...
					const image = await new dig.Facepalm().getImage(`${avatar}`);
					const attachment = new MessageAttachment(image, "fp.png");
					m.delete();
			...
			if (firstColl.size > 0) {
				...
				try {
					...
					const img = await new dig.Facepalm().getImage(`${attach}`);
					const attachment = new MessageAttachment(img, "fp.png");
					m.delete();
					return message.channel.send(attachment);
				} catch (e) {
					m.delete();
