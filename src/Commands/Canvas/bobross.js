...
module.exports = class BobRossCommand extends BaseCommand {
	constructor() {
		super(
			"bobross",
			"canvas",
			...
			"Applies the given image behind a picture of bobross",
			"",
			["SEND_MESSAGES", "EMBED_LINKS"],
			["bobross help` - Sends the help command"],
			["bobross me", "bobross help", "bobross @Tech!", "bobross (attachment)"],
			true,
			5000,
			false,
			false,
		...
		if (args[0]) {
			if (args[0].toLowerCase() == "help") {
				...
				try {
					...
					const attachment = new MessageAttachment(image, "bob_ross.png");
					m.delete();
			...
			if (firstColl.size > 0) {
				...
				try {
					...
					const img = await new dig.Bobross().getImage(`${attach}`);
					const attachment = new MessageAttachment(img, "bob_ross.png");
					m.delete();
					return message.channel.send(attachment);
				} catch (e) {
					m.delete();
