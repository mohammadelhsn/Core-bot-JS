...
module.exports = class ThomasCommand extends BaseCommand {
	constructor() {
		super(
			"thomas",
			"canvas",
			...
			["thomas help` - Sends the help command"],
			["thomas me", "thomas help", "thomas @Tech!", "thomas (attachment)"],
			true,
			5000,
			false,
			false,
		...
		if (args[0]) {
			if (args[0].toLowerCase() == "help") {
				...
				try {
					...
					const image = await new dig.Thomas().getImage(`${avatar}`);
					const attachment = new MessageAttachment(image, "thomas.png");
					m.delete();
			...
			const m = await message.channel.send(gEmbed);
			try {
				const image = await new dig.Thomas().getImage(
			...
			if (firstColl.size > 0) {
				...
				try {
					...
					const img = await new dig.Thomas().getImage(`${attach}`);
					const attachment = new MessageAttachment(img, "thomas.png");
					m.delete();
					return message.channel.send(attachment);
				} catch (e) {
					m.delete();
