...
module.exports = class WastedCommand extends BaseCommand {
	constructor() {
		super(
			...
			"GTA V wasted applied over the given image",
			"",
			["SEND_MESSAGES", "EMBED_LINKS"],
			["wasted help` - Sends the help command"],
			["wasted me", "wasted help", "wasted @Tech!", "wasted (attachment)"],
			true,
			5000,
			false,
			false,
	...
	async run(client, message, args) {
		...
		if (args[0]) {
			...
			try {
				...
				const link = `https://some-random-api.ml/canvas/wasted/?avatar=${img}`;
				const attachment = new MessageAttachment(link, "wasted.png");
				m.delete();
				return message.channel.send(attachment);
			} catch (e) {
			...
			if (firstColl.size > 0) {
				...
				try {
					...
					const link = `https://some-random-api.ml/canvas/wasted/?avatar=${attach}`;
					const attachment = new MessageAttachment(link, "wasted.png");
					m.delete();
					return message.channel.send(attachment);
				} catch (e) {
