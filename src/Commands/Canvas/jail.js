...
module.exports = class JailCommand extends BaseCommand {
	constructor() {
		super(
			...
			["jail help` - Sends the help command"],
			["jail me", "jail help", "jail @Tech!", "jail (attachment)"],
			true,
			5000,
			false,
			false,
		...
		if (args[0]) {
			if (args[0].toLowerCase() == "help") {
				...
				try {
					...
					const image = await new dig.Jail().getImage(`${avatar}`);
			...
			if (firstColl.size > 0) {
				...
				try {
					...
					const img = await new dig.Jail().getImage(`${attach}`);
