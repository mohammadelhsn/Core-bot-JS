...
module.exports = class TrashCommand extends BaseCommand {
	constructor() {
		super(
			...
			["trash help` - Sends the help command"],
			["trash me", "trash help", "trash @Tech!", "trash (attachment)"],
			true,
			5000,
			false,
			false,
	...
	async run(client, message, args) {
		...
		if (args[0]) {
			...
			const m = await message.channel.send(gEmbed);
			try {
				const image = await new dig.Trash().getImage(
			...
			if (firstColl.size > 0) {
				...
				try {
					...
					const img = await new dig.Trash().getImage(`${attach}`);
