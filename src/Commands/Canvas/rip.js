...
module.exports = class RipCommand extends BaseCommand {
	constructor() {
		super(
			...
			["rip help` - Sends the help command"],
			["rip me", "rip help", "rip @Tech!", "rip (attachment)"],
			true,
			5000,
			false,
			false,
		...
		if (args[0]) {
			...
			if (firstColl.size > 0) {
				...
				try {
					...
					const img = await new dig.Rip().getImage(`${attach}`);
					const attachment = new MessageAttachment(img, "rip.png");
					m.delete();
					return message.channel.send(attachment);
				} catch (e) {
					m.delete();
