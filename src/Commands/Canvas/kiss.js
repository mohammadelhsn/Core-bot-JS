...
module.exports = class KissCommand extends BaseCommand {
	constructor() {
		super(
			...
			"Make two people kiss 💋💋💋",
			"",
			["SEND_MESSAGES", "EMBED_LINKS"],
			["kiss help` - Sends the help command"],
			["kiss", "kiss @Core @Tech", "kiss help"],
			true,
			5000,
			false,
			false,
	...
	async run(client, message, args) {
		...
		if (message.mentions.members.size == 2) {
			...
			try {
				const image = await new dig.Kiss().getImage(`${user1}`, `${user2}`);
			...
			const m = await message.channel.send(gEmbed);
			try {
				const image = await new dig.Kiss().getImage(`${member2}`, `${member1}`);
				const attachment = new MessageAttachment(image, "kiss.png");
				m.delete();
				return message.channel.send(attachment);
			} catch (e) {
		...
		if (args[0]) {
			...
			if (firstColl.size > 0) {
				...
				if (secondColl.size > 0) {
					...
					const m = await message.channel.send(gEmbed);
					try {
						const image = await new dig.Kiss().getImage(
							`${attach}`,
							`${attach2}`
						);
						const attachment = new MessageAttachment(image, "kiss.png");
