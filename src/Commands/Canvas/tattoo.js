...
module.exports = class Tattoo extends BaseCommand {
	constructor() {
		super(
			"tattoo",
			"canvas",
			...
			"Applies the given image as the tattoo",
			"",
			["SEND_MESSAGES", "EMBED_LINKS"],
			["tattoo help` - Sends the help command"],
			["tattoo me", "tattoo help", "tattoo @Tech!", "tattoo (attachment)"],
			true,
			5000,
			false,
			false,
		...
		if (args[0]) {
			if (args[0].toLowerCase() == "help") {
				...
				try {
					...
					const image = await new dig.Tatoo().getImage(`${avatar}`);
					const attachment = new MessageAttachment(image, "tattoo.png");
					m.delete();
			...
			const m = await message.channel.send(gEmbed);
			try {
				const image = await new dig.Tatoo().getImage(
			...
			if (firstColl.size > 0) {
				...
				try {
					...
					const img = await new dig.Tatoo().getImage(`${attach}`);
					const attachment = new MessageAttachment(img, "tattoo.png");
					m.delete();
					return message.channel.send(attachment);
				} catch (e) {
					m.delete();
