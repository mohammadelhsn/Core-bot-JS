...
module.exports = class MmsCommand extends BaseCommand {
	constructor() {
		super(
			"mms",
			"canvas",
			...
			"Makes your picture an m&m",
			"",
			["SEND_MESSAGES", "EMBED_LINKS"],
			["mms help` - Sends the help command"],
			["mms me", "mms help", "mms @Tech!", "mms (attachment)"],
			true,
			5000,
			false,
			false,
		...
		if (args[0]) {
			if (args[0].toLowerCase() == "help") {
				...
				try {
					...
					const image = await new dig.Mms().getImage(`${avatar}`);
					const attachment = new MessageAttachment(image, "m&m.png");
					m.delete();
			...
			const m = await message.channel.send(gEmbed);
			try {
				const image = await new dig.Mms().getImage(
			...
			if (firstColl.size > 0) {
				...
				try {
					...
					const img = await new dig.Mms().getImage(`${attach}`);
					const attachment = new MessageAttachment(img, "m&m.png");
					m.delete();
					return message.channel.send(attachment);
				} catch (e) {
					m.delete();
