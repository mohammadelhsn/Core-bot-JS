...
module.exports = class CircleCommand extends (
	BaseCommand
) {
	constructor() {
		super(
			...
			"Changes given image into a circle",
			"",
			["SEND_MESSAGES", "EMBED_LINKS"],
			["circle help` - Sends the help command"],
			["circle me", "circle help", "circle @Tech!", "circle (attachment)"],
			true,
			5000,
			false,
			false,
	...
	async run(client, message, args) {
		...
		if (args[0]) {
			if (args[0].toLowerCase() == "help") {
				...
				const gEmbed = BaseGeneratingEmbed(client, message, self);
				...
				try {
					...
					const image = await new dig.Circle().getImage(`${avatar}`);
			...
			const m = await message.channel.send(gEmbed);
			try {
				const image = await new dig.Circle().getImage(
			...
			if (firstColl.size > 0) {
				...
				try {
					...
					const img = await new dig.Circle().getImage(`${attach}`);
