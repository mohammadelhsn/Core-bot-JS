...
module.exports = class DiscordBlackCommand extends (
	BaseCommand
) {
	constructor() {
		super(
			...
			"Applies given picture with a black discord logo behind it",
			"",
			["SEND_MESSAGES", "EMBED_LINKS"],
			["discordblack help` - Sends the help command"],
			[
				"discordblack me",
				"discordblack help",
				"discordblack @Tech!",
				"discordblack (attachment)",
			],
			true,
			5000,
			false,
		...
		if (args[0]) {
			if (args[0].toLowerCase() == "help") {
				...
				try {
					...
					const image = await new dig.DiscordBlack().getImage(`${avatar}`);
					const attachment = new MessageAttachment(image, "discordblack.png");
					m.delete();
			...
			const m = await message.channel.send(gEmbed);
			try {
				const image = await new dig.DiscordBlack().getImage(
			...
			if (firstColl.size > 0) {
				...
				try {
					...
					const img = await new dig.DiscordBlack().getImage(`${attach}`);
					const attachment = new MessageAttachment(img, "discordblack.png");
					m.delete();
					return message.channel.send(attachment);
				} catch (e) {
					m.delete();
