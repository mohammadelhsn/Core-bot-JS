...
module.exports = class StonkCommand extends BaseCommand {
	constructor() {
		super(
			"stonk",
			"canvas",
			...
			["stonk help` - Sends the help command"],
			["stonk me", "stonk help", "stonk @Tech!", "stonk (attachment)"],
			true,
			5000,
			false,
			false,
		...
		if (args[0]) {
			if (args[0].toLowerCase() == "help") {
				...
				try {
					...
					const image = await new dig.Stonk().getImage(`${avatar}`);
					const attachment = new MessageAttachment(image, "stonk.png");
					m.delete();
			...
			const m = await message.channel.send(gEmbed);
			try {
				const image = await new dig.Stonk().getImage(
			...
			if (firstColl.size > 0) {
				...
				try {
					...
					const img = await new dig.Stonk().getImage(`${attach}`);
					const attachment = new MessageAttachment(img, "stonk.png");
					m.delete();
					return message.channel.send(attachment);
				} catch (e) {
					m.delete();
