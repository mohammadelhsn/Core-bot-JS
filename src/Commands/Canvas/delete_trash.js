...
module.exports = class DeleteTrashCommand extends (
	BaseCommand
) {
	constructor() {
		super(
			...
			["delete help` - Sends the help command"],
			["delete me", "delete help", "delete @Tech!", "delete (attachment)"],
			true,
			5000,
			false,
			false,
		...
		if (args[0]) {
			...
			if (firstColl.size > 0) {
				...
				try {
					...
					const img = await new dig.Delete().getImage(`${attach}`);
