...
module.exports = class BirdfactCommand extends (
	BaseCommand
) {
	constructor() {
		super(
			...
			["birdfact help` - Sends the help command"],
			["birdfact", "birdfact help"],
			true,
			3000,
			false,
			false,
	...
	async run(client, message, args) {
		...
		if (args[0]) {
			...
			try {
				...
				const factEmbed = BaseEmbed(client, message, this)
					.setTitle(`Birdfact command`)
					.setDescription(`Fact: \`${body.fact}\``);
				m.delete();
