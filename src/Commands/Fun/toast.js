...
module.exports = class ToastCommand extends BaseCommand {
	constructor() {
		super(
			"toast",
			"fun",
			[],
			"(help)",
			"Sends you a toast for you to use.",
			"",
			["SEND_MESSAGES", "EMBED_LINKS"],
			["toast help` - Sends the help embed"],
			["toast", "toast help"],
			true,
			3000,
			false,
	...
	async run(client, message, args) {
		...
		try {
			...
			embed.addField("Toast", `\`${await fun.toast()}\``);
			m.delete();
